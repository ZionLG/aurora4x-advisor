                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      {Boolean(cls.Commercial) && (
                        <span className="text-[7px] px-1.5 py-0.5 rounded bg-[var(--cic-green)]/10 text-[var(--cic-green)] border border-[var(--cic-green)]/20">CIV</span>
                      )}
                      {Boolean(cls.MilitaryEngines) && (
                        <span className="text-[7px] px-1.5 py-0.5 rounded bg-[var(--cic-red)]/10 text-[var(--cic-red)] border border-[var(--cic-red)]/20">MIL</span>
                      )}
                    </div>
                  </div>
                  <div className="grid grid-cols-4 gap-2 text-[8px]">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-foreground/60">{((cls.FuelCapacity as number) ?? 0).toLocaleString()}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Jump</span>
                      <p className="font-mono text-foreground/60">{(cls.JumpDistance as number) > 0 ? `${cls.JumpDistance}km` : '—'}</p>
                    </div>
                  </div>
                </div>
              ))}
